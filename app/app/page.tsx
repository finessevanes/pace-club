...
export default function Home() {
  ...
  const handleSuccessfulVerification = async () => {
    ...
    try {
      // Store basic verification success
      ...
      // Try to get onchain verification results by making a request to our API
      try {
        ...
        if (response.ok) {
          ...
          verificationData.onchainVerification = onchainData;
        }
      } catch (error) {
        console.error('Failed to get onchain verification status:', error);
        verificationData.onchainVerification = {
          error: "Failed to check onchain status",
          details: "Could not connect to contract"
        };
      }
      
      // Store the verification results
      localStorage.setItem('verificationResult', JSON.stringify(verificationData));
      
      displayToast("Verification complete! Redirecting...");
      setTimeout(() => {
        router.push("/verified");
      }, 1500);
    } catch (error) {
      console.error('Error processing verification:', error);
      displayToast("Verification successful but processing failed");
      setTimeout(() => {
        router.push("/verified");
      }, 1500);
    } finally {
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 w-full max-w-xs sm:max-w-sm md:max-w-md mx-auto">
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          <div className="text-xs text-gray-600 space-y-1">
            <div><span className="font-medium">Network:</span> {process.env.NEXT_PUBLIC_NETWORK || 'alfajores'}</div>
            <div><span className="font-medium">Contract:</span> {process.env.NEXT_PUBLIC_CONTRACT_ADDRESS ? `${process.env.NEXT_PUBLIC_CONTRACT_ADDRESS.slice(0, 6)}...${process.env.NEXT_PUBLIC_CONTRACT_ADDRESS.slice(-4)}` : 'Not configured'}</div>
          </div>
        </div>
